...
class TSVM:
    def __init__(self, weights = None, C_l = 1.0, C_u = 0.001, kernel = 'rbf', C = 1.0, gamma = 1.0):
        '''
        ...
        self._kernel = kernel
        self._C = C
        self._gamma: str | float = gamma
        self._weights = weights

        self._clf = SVC(C = self._C, kernel = self._kernel, gamma = self._gamma, class_weight = self._weights)
        ...
        while self._C_u < self._C_l:
            ...
            while True:
                ...
                if a > 0 and b > 0 and (a + b) > 2.0:
                    ...
                    # self._Y_u = np.expand_dims(self._Y_u, 1)
                    self._Y = np.concatenate([self._Y_l, self._Y_u])
                    self._clf.fit(self._X, self._Y, sample_weight=sample_weight)
                else:
                    break
